...
{
	if ((*philo).philos_nbr < 1 || (*philo).philos_nbr > 200 || (*philo).die_time < 60)
		return (0);
	else if((*philo).eat_time < 60 || (*philo).sleep_time < 60 || (*philo).must_eat == 0)
		return (0);
	return (1);
}
...
void	init_philos(t_general *general)
{
	...
	while (i < (*general).philos_nbr)
	{
		...
		(*general).philo[i].fork_right = &(*general).forks[(i + 1) % (*general).philos_nbr];
